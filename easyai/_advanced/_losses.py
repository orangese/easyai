...
class StyleRegularizer(keras.regularizers.Regularizer):
    ...
    @staticmethod
    def gram_matrix(a):
        norm_term = 1.0 / K.eval(K.prod(K.int_shape(a)))
...
class ContentRegularizer(keras.regularizers.Regularizer):
    ...
    def __call__(self, x):
        norm_term = self.weight / K.eval(K.prod(K.int_shape(x.output[0])))
        return norm_term * K.sum(K.square(x.output[0] - x.output[1]))
