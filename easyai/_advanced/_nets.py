...
class NSTTransform(ABNetwork):
    ...
    def conv_res_block(filters, filter_size, norm="batch"):
        def _conv_res_block(x):
            identity = keras.layers.convolutional.Cropping2D(cropping=((2, 2), (2, 2)))(
                x)  # 2 is the number of conv layers
...
class NSTLoss(Static):
    ...
    def VGG16(input_tensor):
        ...
        # load weights
        weights_path = keras.utils.data_utils.get_file(NSTLoss.WEIGHT_FILE["vgg16"],
                                                       NSTLoss.WEIGHTS_PATH_NO_TOP["vgg16"],
                                                       cache_subdir="models")
    ...
    def VGG19(input_tensor):
        ...
        # load weights
        weights_path = keras.utils.data_utils.get_file(NSTLoss.WEIGHT_FILE["vgg19"],
                                                       NSTLoss.WEIGHTS_PATH_NO_TOP["vgg19"],
                                                       cache_subdir="models")
        model.load_weights(weights_path, by_name=True)
