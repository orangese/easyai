...
class Noisy_Normalize(keras.layers.Layer):
  ...
  def call(self, x, mask = None):
    ...
    return tf.cast((noise_image * self.noise + x / 255. * (1.0 - self.noise)) / 255., tf.float32)
...
class NST_Transform(Network_Interface):
  ...
  def train_init(self, target_size, noise = 0.6, verbose = True):
    self.num_rows, self.num_cols = target_size
    self.model_init(noise = noise, verbose = verbose)

  def model_init(self, noise, verbose = True):
    self.k_model = NST_Transform.img_transform_net(self.num_rows, self.num_cols, noise = noise)
  ...
  def img_transform_net(img_width, img_height, noise = 0.6, norm = "batch"):
    x = keras.layers.Input(shape = (img_width, img_height, 3))
    a = Noisy_Normalize(noise = noise)(x)

    a = Reflection_Padding2D((40, 40))(a)
    a = NST_Transform.conv_norm_block(32, (9, 9), norm = norm)(a)
    a = NST_Transform.conv_norm_block(64, (3, 3), strides = (2, 2), norm = norm)(a)
    a = NST_Transform.conv_norm_block(128, (3, 3), strides = (2, 2), norm = norm)(a)

    for i in range(5):
      a = NST_Transform.conv_res_block(128, (3, 3), norm = norm)(a)

    a = NST_Transform.conv_norm_block(64, (3, 3), norm = norm, strides = (2, 2), transpose = True)(a)
    a = NST_Transform.conv_norm_block(32, (3, 3), norm = norm, strides = (2, 2), transpose = True)(a)
    a = NST_Transform.conv_norm_block(3, (9, 9), norm = norm, strides = (1, 1), transpose = True)(a)
